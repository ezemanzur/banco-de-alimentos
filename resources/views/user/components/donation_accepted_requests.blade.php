<div class=>
    ...
    @if($listDonations)
        <table class="table table-bordered">
            ...
            <tbody>
            @php $count = 0; @endphp
            @forelse ($donations->where('status', \App\Donation::ESTADO_ACEPTADO) as $d)
                <tr class="list-donations">
                    <th scope="row">
                        {{ $d->myUser()->name }}
                    </th>
                    ...
                    <td class="text-center">
                        <a href="{{route('empleado', ['id' => $d->donation_id,'panel' => 0, 'list' => 0, 'detail' => 1]) }}"
                           class="link">Ver
                            resumen</a>
                    </td>
                    <td>
        ...
        @if($resumeDonation)
            ...
            <div class="resume-donation col-12 " style=" width: 100%;height: 100%">
                <div class="card-header py-2 px-3">
                    Resumen donación:
                    <a class="link close" href=" {{route('empleado', ['panel' => 0]) }} ">
                        Volver al listado
                    </a>
                </div>
